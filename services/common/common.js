...
const imagesProcessing = async (images, ownId) => {
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    const notInterestedImages = await NotInterestedImage.findAll({
      where: { userId: ownId },
    });
    const notInterestedImagesIds = notInterestedImages.map(
      (notInterestedImage) => notInterestedImage.imageId,
    );
    ...
    // likes
    const imageLikes = await ImageLike.findAndCountAll({
    ...
    for (let j = 0; j < comments.rows.length; j++) {
      ...
      const commentLikersIds = commentsLikes.rows.map(
...
const clearOrDeleteOneChatItem = async (chatId, chatterId) => {
  ...
  let chatterNumber;
  if (chat.firstChatterId === chatterId)
    chatterNumber = "visibleForFirstChatter";
  if (chat.secondChatterId === chatterId)
    chatterNumber = "visibleForSecondChatter";
  let updateObject = {};
