...
const { User, Image, Preference, ImageTag, Tag, SavedImage, UserBlocked, UserFollowers, SubscriptionRequest } = require("../models");
...
class userService {
    ...
    setPreferences = async (preferences, userId, isFirstTime) => {
        if (isFirstTime) await User.update({ isFirstTime: false }, { where: { id: userId } });
    ...
    getImagesByPreferences = async (id) => {
        ...
        for (let i = 0; i < Object.keys(frequentlyImagesIds).length; i++) {
            ...
            for (let position of Object.entries(frequentlyImagesIds)) {
                if (position[1] > theMostValue && !sortedFrequentlyImagesIds.includes(position[0])) {
    ...
    getUserProfile = async (ownId, userId) => {
        ...
        ["createdAt", "updatedAt", "password", "popularity"]
            .forEach(key => delete userCopy[key]);
        ...
        if (ownId !== userId && isOpenedForMe) {
            const usualImagesByDate = await this.getImages(ownId, userId, "ordinary", "createdAt");
            const usualImagesByPopularity = await this.getImages(ownId, userId, "ordinary", "popularity");
