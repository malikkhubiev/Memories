...
class chatService {
    ...
    for (let i = 0; i < chats.length; i++) {
      ...
      const user = await User.findOne({
        where: {
          id:
            chat.dataValues.firstChatterId === oneOfChattersId
              ? chat.dataValues.secondChatterId
              : chat.dataValues.firstChatterId,
        },
      ...
      for (let j = 0; j < messages.length; j++) {
        ...
        else message.dataValues.isOwn = false;
  ...
  clearOne = async (chatId, chatterId) => {
    ...
    await Message.update(updateObject, { where: { idOfChat: chatId } });
  };
  deleteOne = async (chatId, chatterId) => {
    const updateObject = await clearOrDeleteOneChatItem(chatId, chatterId);
    await Chat.update(updateObject, { where: { id: chatId } });
