...
class imageService {
  ...
  upload = async (authorId, description, tags, img) => {
    ...
    const image = await Image.create({ authorId, description, fileName });
    Tag.bulkCreate(tags, { ignoreDuplicates: true }).catch((e) =>
  ...
  save = async (userId, imageId) => {
    const maybeAlreadySavedImage = await SavedImage.findOne({
  ...
  unSave = async (userId, imageId) => {
    const maybeAlreadyUnsavedImage = await SavedImage.findOne({
      where: { userId, imageId },
    });
  ...
  view = async (userId, imagesIds) => {
    await Image.increment("popularity", {
      by: 0.5,
      where: {
        id: {
  ...
  like = async (userId, imageId) => {
    try {
      ImageLike.create({ userId, imageId }).catch((e) =>
        console.log(e.message),
      );
