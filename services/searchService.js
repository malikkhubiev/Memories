...
class searchService {
  ...
  getUsersBySubstring = async (id, substring) => {
    ...
    const blockedUsers = await UserBlocked.findAll({
      where: {
        [Sequelize.Op.or]: [{ BlockedId: id }, { userId: id }],
      },
  ...
  getFollowersOrFollowingsBySubstr = async (id, substring, who) => {
    ...
    if (who === "followers") {
      ...
      result = filteredFollowers.map((follower) => {
        ...
        if (followingIds.includes(follower.id)) returnValue.amIFollowed = true;
        return returnValue;
      });
    } else {
  ...
  getBlockedUsersBySubstr = async (id, substring) => {
    ...
    const blockedUsersIds = userBlocked.map((ub) => {
      if (ub.dataValues.userId === id) return ub.dataValues.BlockedId;
  ...
  getImageLikers = async (id, imageId, substring) => {
    ...
    if (substring) {
      const likersIds = [];
      imageLikes.forEach((imageLike) =>
        likersIds.push(imageLike.dataValues.userId),
      );
      const users = await User.findAll({
        where: {
          id: {
