const { User, VerificationCode, Image, ImageTag, Comment } = require("../models");
...
class authService {
    ...
    checkingEmail = async (email, process) => {
        ...
        if (candidate && process === "sign up") throw new Error("User with the same email is already existing");
        if (!candidate && process === "forgot password") throw new Error("There is no such user");
    };
