...
const { signup, deleteAccount, signin, changePassword, saveVerificationCode, checkingEmail, getIsAuth, changeIsOpened, compareCode, changeNameAvatarIsOpened } = require("../services/authService");
...
class authController {
    getIsAuth = async (req, res, next) => {
        try {
            ...
            const { id, name, avatar, isAccountOpened, payload } = await getIsAuth(userId);
    ...
    signin = async (req, res, next) => {
        try {
            ...
            const { id, name, avatar, isAccountOpened, payload } = await signin(email, password);
            const token = generateJwt(payload);
            res.json({ id, name, avatar, isAccountOpened, token });
        } catch (e) {
            next(ApiError.badRequest(e.message || "Something went wrong"));
        ...
        try {
            ...
            if (!errors.isEmpty()) {
                ...
                if (errors.array().length === 1)
                    ...
                    errorMessage = errorsString + "parameters are invalid";
    ...
    checkingEmail = async (req, res, next) => {
        try {
            ...
            if (!email) throw new Error("You didn't provide an email");
            if (!process) throw new Error("You didn't provide what do you want: to sign up / to changePassword");
            await checkingEmail(email, process);
            const code = await sendEmail(email);
            await saveVerificationCode(email, code);
            res.json({ message: "A message has been sent to your mail, read the letter and confirm" });
        } catch (e) {
            next(ApiError.badRequest(e.message || "Something went wrong"));
        };
    };
    ...
    changeNameAvatarIsOpened = async (req, res, next) => {
        try {
            ...
            if (!name && !avatar && !isOpened) throw new Error("You didn't provide any new data");

            const response = await changeNameAvatarIsOpened(id, name, avatar, isOpened);
            res.json(response);
        } catch (e) {
            next(ApiError.badRequest(e.message || "Something went wrong"));
        }
