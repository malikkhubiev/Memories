...
class messageController {
  send = async (req, res, next) => {
    try {
      ...
      // Middleware
      if (!from) throw new Error("You didn't specify sender adress");
      if (!to) throw new Error("You didn't specify recipient adress");
      if (!text) throw new Error("You didn't provide a text");
      if (filter.isProfane(text))
        next(ApiError.badRequest("Your message is profane"));
      // Middleware
  ...
  delete = async (req, res, next) => {
    try {
      ...
      if (!chatId) throw new Error("You didn't specify the message chat");
      if (!messageId)
        throw new Error("You didn't specify which message you want to delete");
