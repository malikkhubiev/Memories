...
const {
  getTagsBySubstring,
  getUsersBySubstring,
  getFollowersOrFollowingsBySubstr,
  getBlockedUsersBySubstr,
  getImageLikers,
...
class searchController {
  ...
  getFollowersOrFollowingsBySubstr = async (req, res, next) => {
    try {
      ...
      substring = substring.slice(1);
      who = who.slice(1);
      if (!who) throw new Error("You didn't provide the user");
  ...
  getImageLikers = async (req, res, next) => {
    try {
      ...
      substring = substring.slice(1);
      if (!imageId) throw new Error("You didn't provide the image");
