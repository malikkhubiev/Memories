...
const { follow, block, unBlock, getUserProfile, getOwnAndFollowingImages, getPreferences, getImages, setPreferences, unFollow, getImagesByPreferences, sendASubscriptionRequest, getRequests, confirmSubRequest, denySubRequest } = require("../services/userService");
...
class userController {
    ...
    setPreferences = async (req, res, next) => {
        try {
            ...
            if (!preferences) throw new Error(
                "You didn't provide the preferences"
            );
    ...
    getImages = async (req, res, next) => {
        try {
            ...
            const images = await getImages(
                ownId, userId, imagesType, typeOfSorting
            );
            res.json(images);
        } catch (e) {
            next(ApiError.badRequest(e.message || "Something went wrong"));
    ...
    sendASubscriptionRequest = async (req, res, next) => {
        try {
            ...
            if (!userId) throw new Error(
                "You didn't provide the user you want to follow"
            );
    ...
    confirmSubRequest = async (req, res, next) => {
        try {
            ...
            if (!userId) throw new Error(
                "You didn't provide the user you want to confirm"
            );
    ...
    denySubRequest = async (req, res, next) => {
        try {
            ...
            if (!userId) throw new Error(
                "You didn't provide the user you want to deny"
            );
    ...
    unFollow = async (req, res, next) => {
        try {
            ...
            if (!followerId) throw new Error(
                "You didn't provide the user you want to unfollow"
            );
    ...
    block = async (req, res, next) => {
        try {
            ...
            if (!blockingId) throw new Error(
                "You didn't provide the user you want to block"
            );
    ...
    unBlock = async (req, res, next) => {
        try {
            ...
            if (!blockerId) next(ApiError.badRequest("No bloker id"));
            if (!unBlockingId) throw new Error(
                "You didn't provide the user you want to unblock"
            );
