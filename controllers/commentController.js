...
class commentController {
  add = async (req, res, next) => {
    try {
      ...
      if (filter.isProfane(text))
        next(ApiError.badRequest("Your comment is profane"));
  ...
  like = async (req, res, next) => {
    try {
      ...
      if (!commentId)
        throw new Error("You didn't specify which comment you want to like");
  ...
  delete = async (req, res, next) => {
    try {
      ...
      if (!commentId)
        throw new Error("You didn't specify which comment you want to delete");
