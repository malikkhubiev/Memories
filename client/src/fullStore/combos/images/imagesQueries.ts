...
export const ownImagesApi = createApi({
  ...
  endpoints: (builder) => ({
    getOwnAndFollowingImages: builder.mutation<imageType[], null>({
      query: () => ({
...
export const useGetOwnAndFollowingImages =
  ownImagesApi.endpoints.getOwnAndFollowingImages.useMutation;
...
export const getUsualImagesByDateThunk = createAsyncThunk(
  ...
  async (userId: number, { rejectWithValue }) => {
    try {
      const response = await $authHost.post<imagesFromServerType>(
        "/user/getImages",
        {
          userId,
          imagesType: "ordinary",
