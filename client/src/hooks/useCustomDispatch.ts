...
const useCustomDispatch = () => {
    ...
    const caller = (something: any, resolveCallback?: any, rejectCallback?: any) => {
        dispatch(setIsLoading(true));
        let dispatchResult = dispatch(something);
        if (dispatchResult.unwrap) dispatchResult = dispatchResult.unwrap();
        dispatchResult.then && dispatchResult.then((result: any) => {
            resolveCallback && resolveCallback(result);
        });
        dispatchResult.catch && dispatchResult.catch((e: ErrorEvent) => {
