...
import { getPrivateImagesByDateThunk, getPrivateImagesByPopularityThunk, getSavedImagesThunk, getUsualImagesByDateThunk, getUsualImagesByPopularityThunk } from '../../../fullStore/combos/images/imagesQueries';
import { selectUsualImagesByDate, selectUsualImagesByPopularity, selectPrivateImagesByDate, selectPrivateImagesByPopularity, selectSavedImages } from '../../../fullStore/combos/images/imagesSlice';
import { useAppDispatch, useAppSelector } from '../../../fullStore/hooks';
import { RootState } from '../../../fullStore/rootStore';
import useCustomDispatch from '../../../hooks/useCustomDispatch';
import { giveCurrentImagesCallbackType, hideShowToggleCallbackType, saveToggleCallbackType, sortByToggleCallbackType } from '../../../types/callbacks';
...
export const OwnProfileButtons: FC<
    OwnProfileButtonsPropsType
> = ({ id, giveCurrentImagesCallback }) => {

    const usualImagesByDate = useAppSelector((state: RootState) => selectUsualImagesByDate(state));
    const usualImagesByPopularity = useAppSelector((state: RootState) => selectUsualImagesByPopularity(state));
    const privateImagesByDate = useAppSelector((state: RootState) => selectPrivateImagesByDate(state));
    const privateImagesByPopularity = useAppSelector((state: RootState) => selectPrivateImagesByPopularity(state));
    const savedImages = useAppSelector((state: RootState) => selectSavedImages(state));
    ...
    const hideShowToggleCallback: hideShowToggleCallbackType = (mode) => {
        if (mode === "hide") {
            ...
            if (isSortingByPopularity) {
                if (privateImagesByPopularity.rows.length) giveCurrentImagesCallback(privateImagesByPopularity);
                else thunksDispatching(getPrivateImagesByPopularityThunk);
            } else {
                if (privateImagesByDate.rows.length) giveCurrentImagesCallback(privateImagesByDate);
                else thunksDispatching(getPrivateImagesByDateThunk);
            };
        } else {
            if (isSortingByPopularity) {
                if (usualImagesByPopularity.rows.length) giveCurrentImagesCallback(usualImagesByPopularity);
                else thunksDispatching(getUsualImagesByPopularityThunk);
            } else {
                if (usualImagesByDate.rows.length) giveCurrentImagesCallback(usualImagesByDate);
    ...
    >
        ...
        <Save
            ...
            saveToggleCallback={saveToggleCallback}
        />
    </Box>
    )
};
