...
import { addInputCallbackType, chatsMenuOptionsActionsType, messageOptionsCallbackType, messagesMenuOptionsActionsType, optionActionCallbackType, setNewChatCallbackType } from '../../../types/callbacks';
...
import { CustomArrowBackIcon, CustomBlockIcon, CustomBroomIcon, CustomDeleteIcon, CustomMoreVerticalIcon } from '../../ui/CustomIcons/CustomIcons';
...
export const Messages: FC<MessagesPropsTypes> = ({
    ...
    useEffect(() => {
        ...
        if (messagesBlockRef.current.scrollHeight -
            messagesBlockRef.current.scrollTop < messagesBlockRef.current.clientHeight * 1.5
            || isSendedMessage)
        ...
        messagesBlockRef.current.addEventListener("scroll", () => {
            if (messagesBlockRef.current.scrollHeight -
                messagesBlockRef.current.scrollTop < messagesBlockRef.current.clientHeight + 50) {
                closeSnackBarHandler();
            };
        });
            ...
            <div className={styles.header}>
                {
                    isSmallSize ?
                        <div
                            onClick={() => setCurrentChat(prev => prev = null)}
                        >
                            <CustomArrowBackIcon />
                        </div> : ""
                }
...
type MessagesPropsTypes = chatType & {
    ...
    customMenuHandler: optionActionCallbackType<
