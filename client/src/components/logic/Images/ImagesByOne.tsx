...
export const ImagesByOne = memo(
  ({ images, idOfSelected, onScrollHomePageFunc }: ImagesByOnePropsType) => {
    ...
    const setImagePosition = useCallback(
      (id: number, position: number, loadedImageHeight: number) => {
    ...
    const menuOptionsHandlerCallback: menuOptionsHandlerCallbackType = (
      ...
      if (action === "Save") {
        usualDispatch(setIsLoading(true));
        saveImage({ imageId: itemId })
          .unwrap()
          .then((fulfilled) => {
            setCurrentImages((prev) => {
              const copy = JSON.parse(JSON.stringify(currentImages));
        ...
        usualDispatch(setIsLoading(true));
        hideImage({ imageId: itemId })
          .unwrap()
          .then((fulfilled) => {
            setCurrentImages(
              (prev) => (prev = prev.filter((image) => image.id !== itemId)),
            );
          })
          .catch((e) => usualDispatch(setErrorMessage(e.data.message)));
        usualDispatch(setIsLoading(false));
