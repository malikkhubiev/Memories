...
import { setIsLoading, setErrorMessage } from '../../../fullStore/combos/user/userSlice';
import { useAppDispatch } from '../../../fullStore/hooks';
import { useAddComment, useDeleteComment } from '../../../fullStore/queries/commentQueries';
import { useGetImageMutation } from '../../../fullStore/queries/imageQueries';
import useSocket from '../../../hooks/useSocket';
import { addInputCallbackType, menuOptionsHandlerCallbackType } from '../../../types/callbacks';
...
export const PostPage: FC<{}> = () => {
    ...
    const isCommentSectionOpenedByParams = useParams().isCommentSectionOpened.slice(1) === "true";
    ...
    let [isCurrentCommentSectionOpened, setIsCurrentCommentSectionOpened] = useState<
        boolean
    >(isCommentSectionOpenedByParams);
    ...
    const menuOptionsHandlerCallback: menuOptionsHandlerCallbackType = (itemId) => {
        usualDispatch(setIsLoading(true));
        deleteComment({ commentId: itemId }).unwrap()
            .then((fulfilled) => {
                let commentsCopy = JSON.parse(JSON.stringify(comments));
                commentsCopy = commentsCopy.filter((comment: commentType) => comment.id !== itemId);
            ...
            }
            {
                isCurrentCommentSectionOpened ?
                    <>
                        ...
                        >
                            {
                                comments && comments.length ? comments.map((comment: any) =>
                                    <Comment
