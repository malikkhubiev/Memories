...
import { useClearOneChat, useDeleteOneChat, useGetChats } from '../../../fullStore/combos/chats/chatsQueries';
...
import { messageOptionsCallbackType, messagesMenuOptionsActionsType, optionActionCallbackType, setNewChatCallbackType } from '../../../types/callbacks';
...
export const ChatsPage: FC<{}> = () => {
        ...
        if (message) {
            ...
            if (currentChat && messageCopy.idOfChat === currentChat.id) {
                if (currentChat.messages) {
                    ...
                    if (foundMessageCopy) {
                        const currentChatCopy = JSON.parse(
                ...
                if (chats.length) {
                    ...
                    chats.forEach((chat: chatType) => {
                        if (
                            ...
                            chat.secondChatterId === userId &&
                            chat.firstChatterId === ownId
                        ) {
                            flag = true;
                            console.log(123)
                            setCurrentChat(
    ...
    > = (action) => {
        if (action === "Clear chat") {
            ...
            const userId = currentChat.firstChatterId === ownId ?
                currentChat.secondChatterId : currentChat.firstChatterId;
    ...
    const messageOptionsCallback: messageOptionsCallbackType = (messageId) => {
        ...
        deleteMessage({
            ...
            .then(() => {
                ...
                const copyOfCurrentChat = JSON.parse(
                    JSON.stringify(currentChat)
                );
