...
import { useFollow, useUnfollow } from '../../../fullStore/combos/profile/profileQueries';
...
export const FollowersPage: FC<{}> = () => {
    ...
    useEffect(() => {
        if (user) {
            setList(prev => prev = isFollowers ?
                // @ts-ignore
                user.followers.list : user.following.list);
            setNumber(prev => prev = isFollowers ?
                // @ts-ignore
                user.followers.number : user.following.number);
        };
    }, [user]);
    ...
    const followCallback: setSelectedUserIdCallbackType =
        (userId) => {
            usualDispatch(setIsLoading(true));
    ...
    const unFollowCallback: setSelectedUserIdCallbackType =
        (userId) => {
            usualDispatch(setIsLoading(true));
    ...
    return (
        ...
        >
            ...
            <SmallGoldenRatioBox>
                <UsersList
                    numberOf={number + " users"}
                    searchHandler={searchHandler}>
                    {
                        list.length ? list.map((user: any) => {
                            if (user.id === ownId)
                                return <RowUser
                                    id={user.id}
                                    avatar={user.avatar}
                                ...
                                return <RowUser
                                    ...
                                    setSelectedUserIdCallback={
                                        user.amIFollowed ?
                                            unFollowCallback : followCallback}
                                    avatar={user.avatar}
                                    name={user.name}
                                    buttonText={
                                        user.amIFollowed ?
