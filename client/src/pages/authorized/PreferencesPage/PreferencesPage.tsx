...
import { CustomPreference, Preference } from '../../../components/logic/Preferences/Preference/Preference';
...
export const PreferencesPage: FC<{}> = () => {
    ...
    const addCustomPref = (preferenceText: string) => {
        ...
        if (trimedPreferenceText[0] === "#") trimedPreferenceText = trimedPreferenceText.slice(1);
        ...
        defaultPreferences.forEach((defpref, index) => {
            if (defpref.text === trimedPreferenceText) {
                ...
                copyOfDefaultPreferences[index] = { ...copyOfDefaultPreferences[index], isSelected: true };
                setDefaultPreferences(prev => prev = copyOfDefaultPreferences);
            };
        });
        if (!found) {
            if (customPreferences.length) {
                setCustomPreferences(prev => prev = [trimedPreferenceText, ...prev]);
            }else{
                setCustomPreferences(prev => prev = [trimedPreferenceText]);
            };
        };
        if (selectedPreferences.length){
            setSelectedPreferences(prev => prev = [trimedPreferenceText, ...prev]);
        }else{
    ...
    const removeCustomPref = (preferenceText: string) => {
        ...
        selectedPreferences.forEach((selectedpref, index) => {
            if (selectedpref === trimedPreferenceText) {
                ...
                return setSelectedPreferences(prev => prev = copyOfSelectedPreferences);
            };
        });
    };
    ...
    const selectPref = (preference: string) => {
        defaultPreferences.forEach((defpref, index) => {
            if (defpref.text === preference) {
                const copyOfDefaultPreferences = [...defaultPreferences];
                copyOfDefaultPreferences.splice(index, 1, { text: defpref.text, isSelected: true });
                setDefaultPreferences(prev => prev = copyOfDefaultPreferences);
            };
        });
        if (selectedPreferences.length){
    ...
    const unSelectPref = (preference: string) => {
        defaultPreferences.forEach((defpref, index) => {
            if (defpref.text === preference) {
                const copyOfDefaultPreferences = [...defaultPreferences];
                copyOfDefaultPreferences.splice(index, 1, { text: defpref.text, isSelected: false });
                setDefaultPreferences(prev => prev = copyOfDefaultPreferences);
            }
        });
        selectedPreferences.forEach((selectedpref, index) => {
    ...
    const nextButtonHanlder = () => {
        const preferencesInServerType = selectedPreferences
            .map((pref: string) => {
                return {
                    name: pref,
                    rating: 0
        ...
        if (preferencesInServerType.length) {
            setPreferences({
                ...
                .unwrap()
                .then(() => {
                    dispatch(setIsFirstTimeToFalse({
                        ownId: id
                    }));
                    navigate("/");
                });
