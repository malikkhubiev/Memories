const {
  getHomepageRecievers,
  getCommentsRecievers,
} = require("../services/wsService");
...
const broadcastConnection = async (aWss, ws, message) => {
  ...
  if (message.data.type === "chats") {
    ...
    return aWss.clients.forEach((client) => {
      if (
        client.imageId === message.data.body.imageId &&
        client.type === message.data.type
      ) {
        const messageWithoutImageId = JSON.parse(
          JSON.stringify(message.data.body),
        );
...
const messagesHandler = (message, ws, aWss) => {
  ...
  switch (message.method) {
    case "connection":
      ...
      if (message.imageId) ws.imageId = message.imageId;
      break;
    case "message":
